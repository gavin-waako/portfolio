...
import getCertifications from "@/libs/getCertifications";
import getDevTools from "@/libs/getDevTools";
import getFrameworksLibraries from "@/libs/getFrameworksLibraries";
...
const Skills1 = ({ type, subTitle, isNotDesc }) => {
  ...
  const frameworksLibraries = getFrameworksLibraries();
  return (
    <section id="skills">
      <div
        className={
      ...
      >
        <div className="container">
          ...
          <div className="frameworks-libraries">
            <div className="flex flex-wrap items-center justify-center gap-x-5 gap-y-30px">
              {/* <!-- skills single --> */}
              {frameworksLibraries?.map(({ name, img, perchant }, idx) => (
                <div
                  key={idx}
                  className="max-w-180px w-full group wow fadeInUp"
                  data-wow-delay={`.${3 + idx}s`}
          ...
          <div className="devtools">
            <div className="flex flex-wrap items-center justify-center gap-x-5 gap-y-30px">
              {/* <!-- skills single --> */}
              {devTools?.map(({ name, img, perchant }, idx) => (
                <div
                  key={idx}
                  className="max-w-180px w-full group wow fadeInUp"
                  data-wow-delay={`.${3 + idx}s`}
          ...
          <div className="certifications">
            <div className="flex flex-wrap items-center justify-center gap-x-5 gap-y-30px">
              {/* <!-- skills single --> */}
              {certifications?.map(({ name, img, perchant }, idx) => (
                <div
                  key={idx}
                  className="max-w-180px w-full group wow fadeInUp"
                  data-wow-delay={`.${3 + idx}s`}
